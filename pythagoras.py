...
class Pythagoras(Scene):
    def construct(self):
        ...
        a_label = Tex("a").move_to(a_line.get_center() + 0.5*DOWN)
        a = VGroup(a_line, a_label)

        b_line = Line(start=[3,0,0], end=[3,4,0]).set_color(GREEN)
        b_label = Tex("b").next_to(b_line.get_center() + 0.05*RIGHT)
        b = VGroup(b_line, b_label)

        c_line = Line(start=[0,0,0], end=[3,4,0]).set_color(RED)
        c_label = Tex("c").move_to(c_line.get_center() + LEFT*0.5)
        ...
        def insert_triangles1(right_triangle):
            ...
            return VGroup(right_triangle1, right_triangle2, right_triangle3, right_triangle4, a_sqr_text, b_sqr_text)

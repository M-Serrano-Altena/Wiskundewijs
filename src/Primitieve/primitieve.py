...
def draw_func(
    ...
    if kwargs.get('integral_range', False):
        ...
        integral = sp.integrate(func(x_symbol), (x_symbol, integral_range[0], integral_range[1]))
        print(f"Area of V = {integral}")

        if kwargs.get("riemann_amt", False):
            plt.fill_between(x, f, where=[(x >= integral_range[0]) and (x <= integral_range[1]) for x in x], color="springgreen", zorder=3, alpha=0.7)
            ...
            if kwargs.get('V_label_loc') is not None and kwargs['V_label_loc']:
                V_label_loc = kwargs['V_label_loc']
                plt.text(s=r"$V$", x=V_label_loc[0], y=V_label_loc[1], color="white", fontsize='xx-large', zorder=4)


    # calculate and plot the roots
    if kwargs.get("root", False):
        x_roots = sp.solve(func(x_symbol))
...
# draw_func(lambda x: 6*x**2, "f(x) = 6x²", x_range=(-2, 2), y_range=(-2, 8), root=False, integral_range=(0, 1), V_label_loc=(0.72, 1.75), func_label=True)

# draw_func(lambda x: sp.cos(0.5*x) + 3, "Oppervlakte onder onbekende functie", x_range=(8, 14), y_range=(-2, 8), root=False, integral_range=(9, 13), V_label_loc=(11, 1.5), func_label=False)
# draw_func(lambda x: sp.cos(0.5*x) + 3, "Oppervlakte onder onbekende functie (met 10 rechthoeken)", x_range=(8, 14), y_range=(-2, 8), root=False, integral_range=(9, 13), riemann_amt=10, func_label=False, arrow_riemann_h=True, arrow_riemann_v=True)
# draw_func(lambda x: sp.cos(0.5*x) + 3, "Oppervlakte onder onbekende functie (met 100 rechthoeken)", x_range=(8, 14), y_range=(-2, 8), root=False, integral_range=(9, 13), riemann_amt=100, func_label=False)

draw_func(lambda x: -x**2 + 2*x + 3 , "f(x) = -x² + 2x + 3", x_range=(-2, 4), y_range=(-2, 6), root=True, integral_range=(0, 3), V_label_loc=(1, 1.5), func_label=True, no_ax=True)